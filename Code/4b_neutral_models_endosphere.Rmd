---
title: "4_neutral_models_endosphere"
author: "Pedro"
date: "9/17/2021"
...
## 4b.0 Neutral models on lists of comparisons
...
# 4b.6 one model for each BO_Root treatment, using all BO_Root samples as a pool

### 4b.6.1 make new physeq object and neutral models
```{r}
...
### 4b.6.2 check metrics of BO_Root neutral models
R², m and fit class percentage metrics can be extracted
```{r}
...
### 4b.6.3 check distribution of the different classes across your listed phyloseq objects
...
#### 4b.6.4 Add neutral fit classes of OTUs to your original phyloseq objects
...
ps_list_treat$Brassica_oleraceae.Root, fit_class_ps_list_BO_Root,
SIMPLIFY = FALSE
)
...
#### 4b.6.5 select only one of the fit classes in BO_Root, then merge the 4 treatment split
...
BO_Root_predicted_list <- subset_ps_above_neutral_below(
  ps_l_predictions_in_taxtable = ps_list_neutral_BO_Root,
  ps_CSS_partition = ps_list_CSS$Brassica_oleraceae.Root
)
```

####  4b.6.6 check stats of treatment effects on each subcommunity
...
# 4b.7 one model for each AT_Root treatment, using all AT_Root samples as a pool

### 4b.7.1 make new physeq object and neutral models
...
### 4b.7.2 check metrics of AT_Root neutral models
R², m and fit class percentage metrics can be extracted
...
### 4b.7.3 check distribution of the different classes across your listed phyloseq objects
...
#### 4b.7.4 Add neutral fit classes of OTUs to your original phyloseq objects
...
ps_list_treat$Arabidopsis_thaliana.Root, fit_class_ps_list_AT_Root,
SIMPLIFY = FALSE
)
...
#### 4b.7.5 select only one of the fit classes in AT_Root, then merge the 4 treatment split
...
AT_Root_predicted_list <- subset_ps_above_neutral_below(
  ps_l_predictions_in_taxtable = ps_list_neutral_AT_Root,
  ps_CSS_partition = ps_list_CSS$Arabidopsis_thaliana.Root
)
```

#### 4b.7.6 check stats of treatment effects on each subcommunity
...
# 4b.8 Join neutral class partitions back together to check possible artefacts


```{r}


# First, make a new phyloseq object containing only the OTUs occuring above prediction
...
## 4b.9.check for artefacts in the methodology
...
## 4b.9.1 run 1000 permanovas on randomly selected OTUs - here, we take teh random otus from inside each tretment and later join them, instead of simply collecing X otus from the full table
...
permanovas_BO_100 <- thousand_permanovas_on_random_otu(
  CSS_fraction = ps_list_CSS$Brassica_oleraceae.Root,
  ps_4_model_l = BO_4_model_above_root
)
permanovas_AT_100 <- thousand_permanovas_on_random_otu(
  CSS_fraction = ps_list_CSS$Arabidopsis_thaliana.Root,
...
#### 4b.9.2 acess the P, F and R2 values of the 100 permanovas for each species, then put it into a single object for both species
...
histogram_artefact_p <- ggplot(
  ...
  geom_vline(
    xintercept = log(full_feature_permanova[[1]][[5]][1]), # p values of AT in the full feature permanova
    linetype = "dotted",
    size = 1.5,
    colour = "#F8766D"
  ) + #  color code for the standard second facotr of a ggplot (teal)
  geom_vline(
    xintercept = log(full_feature_permanova[[3]][[5]][1]), # p values of BO in the full feature permanova
    linetype = "dotted",
    size = 1.5,
    colour = "#00BFC4"
  ) + #  color code for the standard second facotr of a ggplot (teal)
  labs(
    title = "1000 PERMANOVAS, dashed neutral, dotted full, p values",
    x = "Log(p values)",
    y = "Count"
  ) +
  theme_classic() +
...
histogram_artefact_R2 <- ggplot(
  ...
  geom_vline(
    xintercept = full_feature_permanova[[1]][[3]][1], # R2 values of AT in the full feature permanova
    linetype = "dotted",
    size = 1.5,
    colour = "#F8766D"
  ) + #  color code for the standard second facotr of a ggplot (teal)
  geom_vline(
    xintercept = full_feature_permanova[[3]][[3]][1], # R2 values of BO in the full feature permanova
    linetype = "dotted",
    size = 1.5,
    colour = "#00BFC4"
  ) + #  color code for the standard second facotr of a ggplot (teal)
  ...
  labs(
    title = "1000 PERMANOVAS, dashed neutral, dotted full, R2 values",
    x = "R2 values",
    y = "Count"
  ) +
  theme_classic() +
...
histogram_artefact_F <- ggplot(
  ...
  geom_vline(
    xintercept = full_feature_permanova[[1]][[4]][1], # F2 values of AT in the full feature permanova
    linetype = "dotted",
    size = 1.5,
    colour = "#F8766D"
  ) + #  color code for the standard second facotr of a ggplot (teal)
  geom_vline(
    xintercept = full_feature_permanova[[3]][[4]][1], # F2 values of BO in the full feature permanova
    linetype = "dotted",
    size = 1.5,
    colour = "#00BFC4"
  ) + #  color code for the standard second facotr of a ggplot (teal)
  labs(
    title = "1000 PERMANOVAS, dashed neutral, dotted full, F values",
    x = "F values",
    y = "Count"
  ) +
  theme_classic() +
...
# 4b.11 Alpha Diversity of ASVs selected by neutral models
...
ps_AT_alpha_cor_above <- subset_ps_above_neutral_below(
  ps_l_predictions_in_taxtable = ps_list_neutral_BO_Root,
  ps_CSS_partition = ps_list_rarefied$Brassica_oleraceae.Root
)[[1]]

ps_BO_alpha_cor_above <- subset_ps_above_neutral_below(
  ps_l_predictions_in_taxtable = ps_list_neutral_AT_Root,
  ps_CSS_partition = ps_list_rarefied$Arabidopsis_thaliana.Root
)[[1]]
...
# 4b.12  correlate alpha diversity to full dataset to selected ASV dataset
```{r}
